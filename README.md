...
## Critical Architecture Rules (added March 26, 2026)

### _scan_lock rule — IBKR calls from the scheduler

Any function called from the scheduler that uses `ib.reqHistoricalData()`, `get_current_iv()`, `get_call_contracts()`, or any blocking ib_insync call **must run inside `_scan_lock`**. Violating this causes `This event loop is already running` and silent failure.

The pattern (copy from `job_scan_market`):
```python
...
**Never use `nest_asyncio`, `util.patchAsyncio()`, or `asyncio.set_event_loop()` to fix event loop conflicts — they crash the server.**

### Manual assignment recovery procedure

When a put gets marked `EXPIRED` before the wheel detects assignment, create the stock position with ALL three fields:
- `cost_basis` = strike price
- `total_premium_collected` = premium × 100 (missing this crashes `_write_call()` silently)
- `is_wheel = True` (missing this makes `write_covered_calls()` skip the position entirely)

### trade_sync assignment fix (March 26, 2026)

`trade_sync.py` now checks `ibkr_stock_positions` before marking a put `EXPIRED`. If stock appeared, it skips the EXPIRED mark and leaves the put OPEN for the wheel.

### reqAccountUpdates() warning

Portfolio connection uses `reqAccountUpdates()` to stay alive. This keeps the asyncio event loop running. If covered calls fail with event loop errors after any portfolio connection change — this is the first suspect.
