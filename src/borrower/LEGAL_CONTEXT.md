...
**Bilingual templates (record):** The agreement is now maintained in two languages — `loan_agreement_external_v2.md` (English, reviewed) and `loan_agreement_external_v2_et.md` (Estonian). They share Jinja variable names; the operator picks the language per loan on the New Loan form, defaulting to the lender's jurisdiction (Estonian for an Estonian lender, English otherwise — `agreements.default_language_for`). Each generated agreement is executed in a **single** language — only one of the two editions is ever rendered/signed for a given loan — so §15.6 names the chosen language as the governing language. There is deliberately **no** bilingual-execution / "Estonian prevails in case of discrepancy" clause, because no agreement is produced in two languages at once (an earlier draft carried that clause; it was removed as factually incorrect). The Estonian file is a fresh translation of the reviewed English v2 and is **not yet counsel-reviewed**, so it carries version `v2-et-draft` (in `agreements.TEMPLATE_VERSIONS`) and renders with the MUSTAND banner/watermark. Once Estonian counsel has reviewed the translation, drop the `-draft` suffix to promote it to clean/signable, exactly as was done for English.
