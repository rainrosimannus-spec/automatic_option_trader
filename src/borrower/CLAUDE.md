...
Status: data tracking done. **Document attachment storage built** — `loan_documents` table, upload UI on loan detail (PDF-only, ≤ 10 MB, SHA-256 hashed), tied-document rule enforced (`DRAFT → ACTIVE` requires an attached `agreement`). Lender-side download exposes `agreement` + `amendment` only (no `side_letter` / `other`).
...
**E-sign (Digidoc / Dokobit / DocuSign) — not yet built** (Phase 2): to be added as a gated `src/borrower/esign.py` behind `bruno_run_integrations`; on completion it stores the signed PDF as a `loan_documents` agreement and locks the draft (`locked_reason='esign_complete'`), same terminal state as the external path.
