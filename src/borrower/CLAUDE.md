...
Status: data tracking done. **Document attachment storage built** — `loan_documents` table, upload UI on loan detail (≤ 10 MB, SHA-256 hashed), tied-document rule enforced (`DRAFT → ACTIVE` requires an attached `agreement`). Accepts **PDF and Estonian/EU signature containers** (`.asice` / `.sce` / `.bdoc` / `.ddoc`) — validated by extension + magic bytes, canonical MIME derived from extension (browsers send unreliable Content-Type for containers). This lets the already-digitally-signed shareholder loans upload as-is. Lender-side download exposes `agreement` + `amendment` only (no `side_letter` / `other`).

**Contract generation built** (`src/borrower/agreements.py`): clicking *Create Loan* is gated on all agreement variables being present, then auto-generates the filled agreement (markdown → HTML + PDF via weasyprint, amounts-in-words via num2words) and stores it as a `LoanAgreementDraft` row (separate table) + files under `data/agreement_drafts/{loan_id}/v{n}.{md,html,pdf}`. The draft is editable/regenerable (version bumps) until **locked** — locking happens when a signed `agreement` PDF is uploaded (`locked_reason='signed_upload'`; e-sign path reserved). The generated draft is **not** the signed artifact and does **not** satisfy the `DRAFT → ACTIVE` gate — that still requires an uploaded signed PDF. The draft banner + watermark are **review-gated** (`agreements.TEMPLATE_REVIEWED`): they render only while the active template is un-reviewed. The active template is the counsel-reviewed `loan_agreement_external_v2.md` (`TEMPLATE_VERSION = "v2"`, promoted 2026-06-06 on Rain's confirmation), so generated PDFs are **clean and signable**. To re-engage the guard for a future un-reviewed template, give it a `TEMPLATE_VERSION` ending in `-draft`. Company lenders need `Counterparty.represented_by` set before generation. Admin routes: `/borrower/loans/{id}/agreement[.pdf|.html]`, `/agreement/edit`, `/agreement/regenerate`.

**Bilingual (Estonian + English):** the agreement exists as two templates with identical Jinja variable names — `loan_agreement_external_v2.md` (English, reviewed) and `loan_agreement_external_v2_et.md` (Estonian). `resolve_variables(…, language)` localises the prose fields (rate type, payment cadence, terms summary, amount-in-words — Estonian uses a hand-rolled speller since num2words has no `et`, and Estonian number formatting `30 000,00`). The operator picks the language on the New Loan form; it defaults to the lender's jurisdiction (`agreements.default_language_for`: Estonian for an Estonian lender, English otherwise). The chosen template filename is recorded on the draft, so edit/regenerate re-render in the same language (`language_for_template`). The Estonian translation is **not yet counsel-reviewed** (`TEMPLATE_VERSIONS["et"] = "v2-et-draft"`), so it renders with the MUSTAND banner/watermark until promoted (drop the `-draft` suffix). The draft-review guard (`is_template_reviewed`) is per-language.

**E-sign (Dokobit + DocuSign) — scaffolded** (`src/borrower/esign.py`, Phase 2, gated behind `bruno_run_integrations`): Dokobit (Estonian/Baltic eID — Smart-ID/Mobiil-ID/ID-card → `.asice`) and DocuSign (international → signed PDF). `send_for_signature()` no-ops on this dev codebase (logs a skip, returns None); the "Send for e-signature" button on the agreement page surfaces that. The **completion path `register_signed_agreement()` is real and tested everywhere** — it stores the signed artifact as a `loan_documents` AGREEMENT and locks the draft (`locked_reason='esign_complete'`), the same terminal state as a manual upload, so the activation gate behaves identically. Provider request/poll plumbing follows each vendor's documented REST shape but is **not yet validated against a live account** (no creds here): Dokobit send/fetch are wired, DocuSign send/fetch raise a clear "scaffolded, finalize JWT on the clone" error. Creds in `.env` (`DOKOBIT_API_TOKEN`, `DOCUSIGN_*` — see `deploy/bruno.env.example`). No `esign_envelopes` persistence table yet (envelope id only logged) — add when wiring the clone.
