...
class ProposalStatus(str, enum.Enum):
    PENDING = "pending"      # generated by the scheduler, awaiting admin decision
    APPROVED = "approved"    # admin opened the movement-new form, but no real cash flow yet
    REJECTED = "rejected"    # admin declined with a written reason
    EXECUTED = "executed"    # the corresponding LoanMovement has been recorded
...
class LoanRepaymentProposal(Base):
    """Daily-generated recommendation that a loan be repaid early because its
    rate exceeds current trailing trading return (see src/borrower/loan_gates.py
    + src/scheduler/jobs.py job_loan_gate_proposals).

    NOT a money-movement record — those still live in `loan_movements` and are
    only created when an admin actually moves cash via /borrower/movements-new
    (preserving the LHV-reconciliation invariant). Approval here just opens
    that form pre-populated; when the real movement is recorded, the proposal
    flips to EXECUTED and `executed_movement_id` points to the new movement.
    """
    __tablename__ = "loan_repayment_proposals"

    id = Column(Integer, primary_key=True)
    loan_id = Column(Integer, ForeignKey("loans.id"), nullable=False)

    proposed_amount = Column(Float, nullable=False)
    proposed_currency = Column(String(3), nullable=False)
    proposed_date = Column(Date, nullable=False)        # when the recommendation was raised
    reason = Column(Text, nullable=False)               # e.g. "Trading return 8.4% < loan rate 11.55%"

    status = Column(SqlEnum(ProposalStatus), nullable=False, default=ProposalStatus.PENDING)

    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    decided_at = Column(DateTime, nullable=True)
    decided_by_user_id = Column(Integer, ForeignKey("principal_users.id"), nullable=True)
    decided_note = Column(Text, nullable=True)          # mandatory if REJECTED

    # Once admin records a real LoanMovement after approving, point here.
    executed_movement_id = Column(Integer, ForeignKey("loan_movements.id"), nullable=True)
