    ...
    with get_db() as db:
        for item in portfolio_items:
            ...
            if existing:
                ...
                # NOTE: deliberately do NOT record a transaction here. This holdings sync runs on
                # connect (before the trade-sync has recorded the day's fills) and on every trade-sync,
                # so recording a fallback "put_assigned" row for a newly-detected position raced the
                # real "buy" row and produced phantom DUPLICATES (RKLB, MEDP) — and mislabeled direct
                # buys as assignments. Transactions come solely from the trade-sync (real IBKR fills,
                # job_portfolio_sync_trades); genuine option assignments are recorded by the option-side
                # path. Here we only sync the POSITION (the holdings table).
            count += 1
