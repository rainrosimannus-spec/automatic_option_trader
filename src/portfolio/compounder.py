...
def daily_deploy_budget(investable: float, base_pct: float, dca_horizon_days: int,
    ...
    - Base tranche (base_pct of investable) is DCA'd; subtract what's already deployed TODAY
      (`deployed_today`) so the per-DAY pace isn't applied per-scan (the whole base could otherwise
      deploy in 2-3 days — timing risk both ways).
    - LUMP DEFUSAL: the DCA horizon STRETCHES with the size of the undeployed gap. A small routine
      top-up deploys over ~dca_horizon_days; a big new lump (gap ≈ the whole base, e.g. a large
      deposit relative to NLV) stretches toward lump_horizon_days, so the lump averages in over months
      instead of ~1 month — you're never fully deployed right before a crash. Horizon is set from the
      START-OF-DAY gap so the daily pace is stable intraday.
    - FROTH THROTTLE: `pace_throttle` (0..1, set by the caller from SPY vs its 200-day trend) slows the
      base pace when the market is extended above trend (deploy slower into froth, faster after it
      falls). It never throttles the crash dump.
    - When a drawdown tranche is active, deploy the unlocked reserve fast (full remaining gap) — dry
      powder doing its job, NOT subject to the DCA or froth throttle.
...
def single_buy_bounds(nlv: float, cc) -> tuple[float, float]:
    ...
    ~$4M (per-name targets stay under $5k), so each order is sized as a % of CURRENT NLV:
      eff_min = clamp(NLV * min_single_buy_pct, min_single_buy_floor, min_single_buy)  # 0.4%, floor $2k
      eff_max = max(  NLV * max_single_buy_pct, eff_min)                               # 2%
    eff_min is a HARD $2,000 floor that holds flat up to ~$500k NLV, then SCALES UP with NLV
    (min_single_buy_pct = 0.4%, so 0.4% × $500k = $2,000) — uncapped, like eff_max. No order is ever
    smaller than eff_min; a name whose target gap is below it is skipped until its target grows. So:
      eff_min = max(NLV * 0.4%, $2,000)   # flat $2k <= $500k, then 0.4% of NLV
      eff_max = max(NLV * 2%,  eff_min)   # 2% of NLV
    Below ~$100k NLV eff_min == eff_max == $2k (every order exactly the floor). min stays ~20% of max
    once both scale.
    """
    lo = max(nlv * cc.min_single_buy_pct, cc.min_single_buy_floor)
